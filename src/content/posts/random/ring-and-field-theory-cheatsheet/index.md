...
在整数 $\Z$ 上有一个可计算的带余除法 $m = m \mathbin\mathrm{div} n + m \mathbin\mathrm{mod} n$ . 域 $F$ 为系数的多项式环 $F[x]$ 上，在 $\deg f(x) \ge \deg g(x)$ 时也可以用 $g(x)$ 对 $f(x)$ 计算多项式带余除法。思考一下，会发现在一般的整环上，定义带余除法有一点困难。首先，我们不一定能保证这样的分解存在。其次，「越除越大」的可能性没有禁止！

### Euclidean 度量

先解决后一个问题。注意到我们此前对环的研究完全没有引入过任何大小关系，所以我们需要人为定义一种比较大小的方法，才能去要求余数比除数小。

> 整环 $D$ 的一个 **Euclidean 度量** 是一个函数 $d : D \to \N$ , 满足
> 
> - $x : D = 0 \iff d(x) = 0$ ;
> 
> - $d(ab) \ge d(a) \space \forall a,b \ne 0$ .

有些作者会把 Euclidean 度量定义在 $D - \left \{ 0 \right \} \to \N$ 上，这两种定义是等价的，整体平移 $1$ 即可。

现在我们可以比较整环中的元素了。在 $\Z$ 上，度量可以直接定义为绝对值函数。在多项式上就比较麻烦，定义
...
这时如果采用 $D - \left\{ 0 \right\} \to \N$ 版本的度量函数就会更方便，直接定义成 $\deg$ 即可。注意这还要求多项式环 $F[x]$ 的系数 $F$ 是域。

一个自然的问题是为什么要求 $D \to \N$ 而非像测度那样 $\R_{\ge0}$ 或者别的什么上。这是因为我们不希望出现 ***无穷下降链*** ，因为我们需要带余除法总能计算出结果来。

> **定义** 一个整环 $D$ 是 **Euclidean 整环 (ED)** , 当且仅当其上有一个 Euclidean 度量 $d$ , 以及一个带余除法算法，对 $a, b : D$ 在 $b \ne 0$ 时给出 $a = a \mathbin\mathrm{div} b + a \mathbin\mathrm{mod} b$ 使得 $d(a \mathbin\mathrm{mod} b) < d(b)$ .
